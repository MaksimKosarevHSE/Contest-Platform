...
public interface ContestRepository extends JpaRepository<Contest, Integer> {
    ...
    Problem getProblem(Integer contestId, Integer problemId);


    @Query("select new com.maksim.problemService.dto.ContestSignatureDto(c.id, c.authorId, c.startTime, c.endTime, c.isRunning) from Contest c order by c.startTime desc")
    List<ContestSignatureDto> getAll(PageRequest of);

    @Query("select new com.maksim.problemService.dto.ContestSignatureDto(cu.contest.id, cu.contest.authorId, cu.contest.startTime, cu.contest.endTime, cu.contest.isRunning) from ContestUser cu where cu.id.userId = :userId order by cu.contest.startTime desc")
    ...
    ProblemConstraints getProblemConstraints(Integer contestId, Integer problemId);
}
